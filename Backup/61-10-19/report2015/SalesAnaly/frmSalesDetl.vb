...
Public Class frmSalesDetl
    ...
    Function getSQL() As String
        ...
        txtSQL = txtSQL & "sum(Trh_Amt-Trh_Disc_Amt)as Trh_Amt,sum(Trh_W_Sum)as Trh_W_Sum,sum(Trh_Amt-Trh_Disc_Amt-Trh_Cost_Amt)as Profit "
        txtSQL = txtSQL & "From TranDataH "
        If optCusGrp.Checked = True Then
            txtSQL = txtSQL & "Left Join ArFile "
        ...
        txtSQL = txtSQL & "sum(Trh_Amt-Trh_Disc_Amt)as Trh_Amt59,sum(Trh_W_Sum)as Trh_W_Sum59,sum((Trh_Amt-Trh_Disc_Amt)-Trh_Cost_Amt)as Profit59 "
        ...
        If optSum.Checked = True Then
            txtSQL = txtSQL & "Year(Trh_Date)='2016')  " ' อนาคตต้องมาเปลี่ยนให้เป็น  year(Trh_Date)=year(GetDate())-2
        ElseIf optAVG_Now.Checked = True Then
            txtSQL = txtSQL & "Year(Trh_Date)='2016'  " ' อนาคตต้องมาเปลี่ยนให้เป็น  year(Trh_Date)=year(GetDate())-2
        ...
        txtSQL = txtSQL & "(sum(Trh_Amt)/sum(Trh_W_Sum))as Pr_Kg "
        txtSQL = txtSQL & "From TranDataH "
        txtSQL = txtSQL & "Left Join ArFile "
        txtSQL = txtSQL & "ON Trh_Cus=Ar_Cus_ID "
        ...
        End If
        txtSQL = txtSQL & "Ar_Name,AR_LastSales_Date,Ar_Target_Year,Ar_Target,Ar_Sales_Target,PrKg.Pr_Kg,TrH59.Trh_Amt59,Profit59 "
        ...
        ElseIf optTotalSales.Checked = True Then
            If optAZ.Checked = True Then
                txtSQL = txtSQL & "Order by Trh_Amt ASC "
            Else
                txtSQL = txtSQL & "Order by Trh_Amt DESC "
            End If
    ...
    Sub runRpt()
        ...
        With subDS.Tables("slDeTail")
            ...
            If optAVG_Now.Checked = True Then
                lbKPI.Text = Format(getSaleKPI(selSale) * (Month(Now) - 1), "#,##0.00")
            ...
            For i = 0 To subDS.Tables("slDetail").Rows.Count - 1
                ...
                If IsDBNull(.Rows(i).Item("Trh_Amt59")) Then
                    arSumSales59 = 0
                Else
                    arSumSales59 = .Rows(i).Item("Trh_Amt59")
                ...
                End If
                If IsDBNull(.Rows(i).Item("Trh_Amt")) Then
                    arAmt = 0
                    arSumQtyW = 0
                    arPrKg = 0
                Else
                    arAmt = .Rows(i).Item("Trh_Amt")
    ...
    Function getSQLshowChart() As String
        ...
        If optGrade.Checked = True Then
            ...
            txtSQL = txtSQL & "isnull(sum(Profit),0)as Profit,sum(Trh_Amt)as Trh_Amt, "
        ...
        txtSQL = txtSQL & "( Select sum(Trh_Amt-Trh_Disc_Amt-Trh_Cost_Amt)as sumProfit "
    ...
    Function genSQLMonthChart() As String
        ...
        txtSQL = txtSQL & "isnull(sum((Trh_Amt -Trh_Disc_Amt- Trh_Cost_Amt)), 0)As Profit  "
    ...
    Function genSQLGrpProD()
        ...
        txtSQL = txtSQL & "order by Trh_Amt "
        Return txtSQL
