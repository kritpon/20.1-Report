...
Public Class frmSales_ReportByTT
    ...
    Sub formatGrid()
        ...
        lsvARdetail.Columns.Add("ยอดกำไรปี 60", 120, HorizontalAlignment.Right)
        ...
        'lsvARdetail.Columns.Add("Target" & Year(Now), 120, HorizontalAlignment.Right)
        lsvARdetail.Columns.Add("ยอดกำไรปี 61", 120, HorizontalAlignment.Right)
    ...
    Function getSQL() As String
        ...
        txtSQL = txtSQL & "sum(case when (year(Trh_Date)='" & (Year(Now) - 543) - 1 & "')  then (Trh_Amt-Trh_Cost_Amt) else 0 end)as 'TrhProfit" & (Year(Now) - 1).ToString & "', "
        txtSQL = txtSQL & "sum(case when (year(Trh_Date)='" & (Year(Now) - 543) - 0 & "')  then (Trh_Amt-Trh_Cost_Amt) else 0 end)as 'TrhProfit" & (Year(Now) - 0).ToString & "', "
        txtSQL = txtSQL & "sum(Trh_Amt-Trh_Cost_Amt)as sumDiff "
        ...
        If optOrProfit60.Checked = True Then
            If optOrAZ.Checked Then
                txtSQL = txtSQL & "Order by TrhProfit2560 asc "
            Else
                txtSQL = txtSQL & "Order by TrhProfit2560 desc "
            End If


        ElseIf optOrProfit61.Checked = True Then
            If optOrAZ.Checked Then
                txtSQL = txtSQL & "Order by TrhProfit2561 asc "
            Else
                txtSQL = txtSQL & "Order by TrhProfit2561 desc "
            End If
    ...
    Sub runRpt()
        ...
        For i = 0 To subDS.Tables("slDetail").Rows.Count - 1
            ...
            With subDS.Tables("slDetail").Rows(i)
                ...
                If IsDBNull(.Item("TrhProfit2560")) Then
                    dblTrhProfit60 = 0 '.Item("TrhProfit60")
                Else
                    dblTrhProfit60 = .Item("TrhProfit2560")
                    dblTotalTrhProfit60 = dblTrhProfit60 + dblTotalTrhProfit60

                End If
                If IsDBNull(.Item("TrhProfit2561")) Then
                    dblTrhProfit61 = 0
                Else
                    dblTrhProfit61 = .Item("TrhProfit2561")
