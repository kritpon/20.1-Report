...
Module dbTools
    ...
    Public selOptSL As Integer  '(ถ้า  seloptsl=0 เป็นรายงาน  CS   ,  ถ้าเป็น 1  เป็นรายงาน Sales
    ...
    Function getSaleKPI(ByVal saleId As String, selOptCS_SC As Integer) As Double
        Dim ans As String
        Dim subDA As SqlClient.SqlDataAdapter
        Dim subDS As New DataSet
        ...
        'if 0 คือ  CS 
        'if 1  คือ SAles
        If selOptCS_SC = 0 Then

            txtSQL = "Select * "
            txtSQL = txtSQL & "From CSMast "
            txtSQL = txtSQL & "WHERE CS_Code='" & saleId & "'"
            ...
            If subDS.Tables("slList").Rows.Count - 1 < 0 Then
                ans = 0
            Else
                ans = subDS.Tables("slList").Rows(0).Item("CS_KPI")
            End If
        Else
