...
Public Class frmAnalyCus
    ...
    Sub RunFrmLoad()
        ...
        If frmLoad = False Then
            ...
            With subDS.Tables("arFile1")
                ...
                End If
                If IsDBNull(.Rows(0).Item("Ar_Grade")) Then
                    cboSelGrade.Text = "-"
                Else
                ...
                '====  ส่วน Target ======
                lbGPAmt.Text = Format(.Rows(0).Item("Ar_GP_SL_Target"), "#,##0.00")
                lbGPProfit.Text = Format(.Rows(0).Item("Ar_GP_Profit"), "#,##0.00")
                lbGPweight.Text = Format(.Rows(0).Item("Ar_GP_W_Target"), "#,##0.00")
                lbTTAmt.Text = Format(.Rows(0).Item("Ar_TT_SL_Target"), "#,##0.00")
                lbTTProfit.Text = Format(.Rows(0).Item("Ar_TT_Profit"), "#,##0.00")
    ...
    Function genSQLChartMonth() As String
        ...
        If optPrice.Checked = True Then
            'txtSQL = txtSQL & "month(Trh_Date)as Month,sum(Trh_D_Amt)as sumAmt,sum(Trh_W_Sum)as SumQtyW "
            txtSQL = txtSQL & " year(trh_date)as Y,month(trh_date)as M ,sum(isnull(trh_D_amt,0))as Trh_D_Amt,"
        ...
        Else
            ...
            txtSQL = txtSQL & " year(trh_date)as Y,month(trh_date)as M ,sum(Trh_Cost_Amt)as Trh_Cost,"
            txtSQL = txtSQL & "sum(isnull(trh_D_amt-Trh_Disc_Amt,0))as Trh_D_Amt,sum(isnull(trh_w_sum,0))as Trh_W_Sum  "
            ' txtSQL = txtSQL & ",isnull(sum(trh_D_amt)/sum(trh_w_sum),0)as price "
        ...
        For i = 0 To subDS.Tables("saleMonth").Rows.Count - 1
            ...
            If optPrice.Checked = True Then
                ...
                With subDS.Tables("saleMonth").Rows(i)
                    ...
                    dblPrice = (.Item("Trh_D_Amt") / .Item("Trh_W_Sum"))
    ...
    Function genSQLGrpProD()
        ...
        txtSQL = txtSQL & "sum(isnull(trh_w_sum,0))as Trh_W_Sum, "
        txtSQL = txtSQL & "isnull(sum(case when (Trh_ProD_Sales='01') then ((Trh_D_Amt)/(trh_W_Sum))end ),0 ) as price "
    ...
    Sub runGetTarget()
        ...
        txtSQL = txtSQL & "sum(Trh_Cost_Amt)as Trh_Cost_Amt,sum(Trh_D_Amt)as Trh_D_Amt,"
        txtSQL = txtSQL & "(((sum(Trh_D_Amt-Trh_Disc_Amt)-sum(Trh_Cost_Amt))/sum(Trh_Cost_Amt))*100)as Ratio "
